-- 生成 2026-01-01 00:00:00 至 2026-01-19 23:59:30，每30秒一条（MySQL 5.7 兼容）
    ...
    FROM (
      ...
      FROM (
        SELECT
          ...
          DATE(IF(HOUR(TIMESTAMP('2026-01-01 00:00:00') + INTERVAL (n * 30) SECOND) < 12, 
                  DATE_SUB(TIMESTAMP('2026-01-01 00:00:00') + INTERVAL (n * 30) SECOND, INTERVAL 1 DAY), 
                  TIMESTAMP('2026-01-01 00:00:00') + INTERVAL (n * 30) SECOND)) AS ref_date
        FROM (
        ...
        WHERE n <= (25 * 24 * 60 * 60 / 30) -- 确保覆盖到 2026-01-19 23:59:30
      ) base
    ) calc
  ) ranges
) s;
